...
export default function PeoplePage() {
  return <main className="people-page">
    ...
    <section className="people-directory" aria-labelledby="team-directory-heading"><div className="people-directory-heading"><p className="section-kicker">MY STORY DETECTIVE</p><h2 id="team-directory-heading">Meet the team.</h2></div><div className="people-directory-grid">{team.map(person => <article key={person.name}><div className="portrait"><img src={person.image} alt={`Portrait of ${person.name}`} /></div><h3>{person.name}</h3><h4>{person.projectRole}</h4><p className="person-occupation">{person.title}</p><p>{person.bio}</p><div className="person-links" aria-label={`${person.name}'s professional links`}>{person.website && <a href={person.website} target="_blank" rel="noreferrer" aria-label={`${person.name}'s website`} title="Website"><WebsiteIcon /></a>}{person.linkedin && <a href={person.linkedin} target="_blank" rel="noreferrer" aria-label={`${person.name}'s LinkedIn profile`} title="LinkedIn"><LinkedInIcon /></a>}</div></article>)}</div></section>
    <SiteFooter />
  </main>;
}
